...
// 操作数 寻址方式
type operand =
    | Imm of imm (* immediate 立即寻址 *) 
    | Reg of reg (* register 寄存器寻址 *)
    | Ind1 of imm (* indirect: displacement PC相对寻址 *)
    | Ind2 of reg (* indirect: (%reg) 寄存器间接 *)
    | Ind3 of (imm * reg) (* indirect: displacement(%reg) 寄存器相对 *)
