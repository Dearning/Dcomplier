...
let rec type_to_string (e: typ) : string =
    match e with
    | TVar x -> "'" + x
    | Arrow (Arrow (u, v) as s, t) -> 
        Printf.sprintf "(%s) -> %s" (type_to_string s) (type_to_string t)
    | Arrow (s, t) -> 
        Printf.sprintf "%s -> %s" (type_to_string s) (type_to_string t)
...
let rec aexpr_to_string (e: aexpr) : string =
    match e with
    | AVar (x, a) -> 
        Printf.sprintf "(%s:%s)" x (type_to_string a)
    | AApp (e1, e2, a) -> 
        Printf.sprintf "((%s %s):%s)" (aexpr_to_string e1) (aexpr_to_string e2) (type_to_string a)
    | AFun (x, e, a) -> 
        Printf.sprintf "((Fun %s -> %s):%s)" x (aexpr_to_string e) (type_to_string a)
