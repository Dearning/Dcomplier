...
let verb msg = (if verbose.contents then (printf "%s" msg))
...
let configure () =
    if isWindows then
             (verb "platform = windows\n"
              as_cmd.contents <- "gcc -c -o "
              link_cmd.contents <- "gcc -o ")
    else
        (if isLinux then
             (verb "platform = linux\n"
              as_cmd.contents <- "clang -c -o "
              link_cmd.contents <-  "clang -o ")
         else
             (verb "platform = OS X\n"))
        //  try
        //      ignore (stat output_path.contents)
        //  with Unix_error (ENOENT, _, _) ->
        //      (verb
        //       <| Printf.sprintf "creating output directory: %s\n" output_path.contents)

        //      mkdir output_path.contents 0o755) //TODO
...
let clang_compile (dot_ll: string) (dot_s: string) : unit =
    sh (sprintf "%s%s %s" clang_cmd.contents dot_s dot_ll) raise_error

let assemble (dot_s: string) (dot_o: string) : unit =
    sh (sprintf "%s%s %s" as_cmd.contents dot_o dot_s) raise_error

let preprocess (dot_oat: string) (dot_i: string) : unit =
    sh
        (sprintf "%s%s %s %s" pp_cmd.contents (List.fold (fun s i -> s ^ " -I" ^ i) "" include_paths.contents) dot_oat dot_i)
        raise_error
...
let link (mods: string list) (out_fn: string) : unit =
    sh
        (sprintf
            ...
            link_cmd.contents
            out_fn
            (String.concat " " (mods @ lib_paths.contents))
            (List.fold (fun s i -> s ^ " -L" ^ i) "" lib_search_paths.contents)
            (List.fold (fun s i -> s ^ " -I" ^ i) "" include_paths.contents)
            (List.fold (fun s l -> s ^ " -l" ^ l) "" libs.contents))
        raise_error
