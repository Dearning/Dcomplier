...
module IR =
    ...
    module MLMeaning =
        ...
        let load (x: int64 ref) = (x.Value)
        let store o (x: int64 ref) = x.Value <- o
...
module Compile =
  ...
  *)
    ...
    let rec compile_exp (e: exp) : (insn list) * opn =
        ...
        (match e with
         | SRC.Var x -> let ret = mk_uid "tmp" in [ Load(ret, x) ], IR.Id ret
    ...
    let rec compile_cmd (c: cmd) : stream =
        (match c with
         ...
         | Skip -> []
         //赋值语句
         | Assn (v, e) -> let (is, op) = compile_exp e in (lift is) >+ I(Store(v, op))
         //顺序语句
